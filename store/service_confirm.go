...
func (s *SQLite) CutConfirm(ctx context.Context, id task.ID, req CutConfirmRequest) (CutConfirmResult, error) {
	...
	err := s.withTx(ctx, func(tx *sql.Tx) error {
		...
		if err := task.ValidateCutConfirmation(task.CutConfirmation{
			PersonnelID: string(req.PersonnelID),
			Snapshot:    req.Snapshot,
		}, row.snapshot(), personnelStrings(confirmed)); err != nil {
			if errors.Is(err, task.ErrDuplicateCutter) {
				return errDuplicate("duplicate cutter", err.Error())
			}
			return errInvalid("snapshot mismatch", err.Error())
		}
